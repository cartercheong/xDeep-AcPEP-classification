...
def generatePSSMProfile(fastas, outDir, blastpgp, db):
	...
	for i in fastas:
		...
		with open(name + '.txt', 'w') as f:
			f.write('>'+name+'\n'+sequence + '\n')
		myCmd = blastpgp + ' -a ' + '5' + ' -i ' + name + '.txt' + ' -d ' + db + ' -b 0 -v 5000 -j 3 -h 0.001 -Q ' + outDir + '/' + name +'.pssm'
		#myCmd = blastpgp + ' -query ' + name + '.txt' + ' -db ' + db + ' -num_alignments 0 -num_descriptions 5000 -inclusion_ethresh 1000 -num_threads 3 -num_iterations 3 -use_sw_tback -out_ascii_pssm ' + outDir + '/' + name + '.pssm'
